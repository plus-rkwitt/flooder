new p5(p => {
  ...
  // CHOOSE DATASET HERE:
  // -----------------------------
  const DATASET = "coral";  // "coral", "lockwasher" or "virus"
  // -----------------------------
  ...
  // -----------------------------
  p.setup = () => {
    const container = document.getElementById('canvas-coral');
    if (!container) return;
  ...
  // BUTTONS
  // -----------------------------
  document.getElementById('prevBtn_coral').addEventListener('click', () => {
    tIndex = Math.max(0, tIndex - 1);
    drawSnapshot();
  });

  document.getElementById('nextBtn_coral').addEventListener('click', () => {
