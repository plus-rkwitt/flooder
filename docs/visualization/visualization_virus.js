new p5(p => {
  ...
  // CHOOSE DATASET HERE:
  // -----------------------------
  const DATASET = "virus";  // "coral", "lockwasher" or "virus"
  // -----------------------------
  ...
  // -----------------------------
  p.setup = () => {
    const container = document.getElementById('canvas-virus');
    if (!container) return;
  ...
  // BUTTONS
  // -----------------------------
  document.getElementById('prevBtn_virus').addEventListener('click', () => {
    tIndex = Math.max(0, tIndex - 1);
    drawSnapshot();
  });

  document.getElementById('nextBtn_virus').addEventListener('click', () => {
